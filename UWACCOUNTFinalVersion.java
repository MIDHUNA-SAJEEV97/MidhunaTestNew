...
public class UWACCOUNTFinalVersion extends BaseClass {
...
//            WebElement subYes = wait.until(ExpectedConditions.visibilityOfElementLocated(By.id(subsidiaryRadioPrefixYes)));
//            jsClick(subYes);
...
//            WebElement subNo = driver.findElement(By.id(subsidiaryRadioPrefixNo));
//            jsClick(subNo);
...
////        //extracting the list of names from dropdown for adding in excel(onetime purpose)
//        List<String> claimCauseNames = select.getOptions().stream()
//                .filter(o -> {
//                    String val = o.getAttribute("value");
//                    String text = o.getText().trim();
//                    return val != null && !val.isEmpty() && !text.equalsIgnoreCase("Please select...");
...
//                .map(o -> o.getText().trim())
//                .toList();
//        System.out.println("The list of ClaimCause names are :"+ claimCauseNames);
//        //-----------------------------------------------------
...
//            // Create Select object for dropdown
//            Select select1 = new Select(portableComputerExcessTheftMaliciousDamage);
//            String cleanName1 = d.PortableComputerExcessTheft                   // Clean Excel string (critical fix)
//                    .trim()
//                    .replace("\u00A0", "")      // remove non-breaking spaces
//                    .replaceAll("\\s+", " ");  // normalize multi-spaces
//            try {
//                select1.selectByVisibleText(cleanName1);                // Try direct select first
//            } catch (Exception e) {
//                // Fallback: loop through all options and match ignoring case
//                for (WebElement option : select1.getOptions()) {
//                    if (option.getText().trim().equalsIgnoreCase(cleanName1)) {
//                        option.click();
//                        break;
//                    }
//                }
...
//            // Create Select object for dropdown
//            Select select2 = new Select(portableComputerExcessAllOtherClaims);
//            String cleanName2 = d.PortableComputerExcessOtherClaims                   // Clean Excel string (critical fix)
//                    .trim()
//                    .replace("\u00A0", "")      // remove non-breaking spaces
//                    .replaceAll("\\s+", " ");  // normalize multi-spaces
//            try {
//                select2.selectByVisibleText(cleanName2);                // Try direct select first
//            } catch (Exception e) {
//                // Fallback: loop through all options and match ignoring case
//                for (WebElement option : select2.getOptions()) {
//                    if (option.getText().trim().equalsIgnoreCase(cleanName2)) {
//                        option.click();
//                        break;
//                    }
//                }
